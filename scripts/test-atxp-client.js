...
async function testATXPClient(url) {
	...
	try {
		...
		if (!connectionString) {
			console.error(
				"❌ ATXP_CONNECTION_STRING environment variable is required",
			);
			console.error("");
			console.error("Please set up your .env file with:");
			console.error(
				"ATXP_CONNECTION_STRING=atxp://accountId@network/privateKey",
			);
			console.error("");
			console.error("Example:");
			console.error(
				"ATXP_CONNECTION_STRING=atxp://my-account@solana-devnet/your_base58_private_key",
			);
			process.exit(1);
		}
		...
		const client = await atxpClient({
			...
			onAuthorize: async ({ authorizationServer, userId }) => {
				console.log(
					`🔐 Authorized with server ${authorizationServer} as user ${userId}`,
				);
			},

			onAuthorizeFailure: async ({ authorizationServer, userId, error }) => {
				console.error(
					`❌ Authorization failed for user ${userId} on ${authorizationServer}:`,
					error.message,
				);
			},
			...
			onPayment: async ({ payment }) => {
				console.log(
					`✅ Payment successful: ${payment.amount} ${payment.currency}`,
				);
			},

			onPaymentFailure: async ({ payment, error }) => {
				console.error(
					`❌ Payment failed: ${payment.amount} ${payment.currency} - ${error.message}`,
				);
			},
		});
		...
		toolsResponse.tools.forEach((tool) => {
			console.log(
				`   • ${tool.name}: ${tool.description || "Payment-protected tool"}`,
			);
		});
		console.log("");
		...
		console.log(
			"💡 The server successfully handled payment requirements and tool execution.",
		);
	} catch (error) {
		...
		if (
			...
			console.log(
				"💡 This might be expected behavior if testing real payment flows.",
			);
...
async function testHelloWorldWithPayment(client) {
	...
	console.log(
		'   📞 Calling hello_world({ name: "ATXP User" }) - expect payment flow...',
	);
	try {
...
function showUsage() {
	...
	console.log(
		"  node scripts/test-atxp-client.js                                    # Test remote server (default)",
	);
	console.log(
		`  node scripts/test-atxp-client.js ${LOCAL_URL}     # Test local server`,
	);
	console.log(
		`  node scripts/test-atxp-client.js ${REMOTE_URL}    # Test remote server`,
	);
	console.log("");
